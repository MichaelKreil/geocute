...
function save(filename, count, dataX, dataY, dataV) {
	...
	function hilbertSort() {
		...
		index.sort((a,b) => {
			...
			return compare(xa,ya,xb,yb,0);

			return result;

			function compare(xa,ya,xb,yb,cell) {
				var ida = (xa >>> 15) + 2*(ya >>> 15);
				var idb = (xb >>> 15) + 2*(yb >>> 15);
				
				if (ida === idb) {
					return compare(frac(xa), frac(ya), frac(xb), frac(yb), hilbertSubCell[cell][ida]);
				}

				return hilbertOrder[cell][ida] - hilbertOrder[cell][idb]
