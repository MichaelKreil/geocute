...
const x0 =  0, x1 = 20;
const y0 = 40, y1 = 60;
const xs = 20000000/(x1-x0);
const ys = 20000000/(y1-y0);

function load(filename) {
	let buffer = fs.readFileSync(filename);
	buffer = zlib.brotliDecompressSync(buffer);
	
	let count = Math.round(buffer.length/10);
	...
	function readUint16(offset, map) {
		let input = new Uint16Array(count);
...
function save(filename, count, dataX, dataY, dataV) {
	...
	var buffer = Buffer.alloc(10*count);
	...
	fs.writeFileSync(filename, zlib.brotliCompressSync(buffer, { level:9 }));

	function hilbertSort() {
		let index = new Uint32Array(count);
		let hash  = new Array(count);
